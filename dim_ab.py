...
from utils import *
from model_utils import MyModel_random, MyModel_true
...
def exp(s, model_name):
    ...
    for f in range(n_folds):
        ...
        if model_name == 'learn':
            ...
            from model_utils import MyModel_multiple, custom_loss
            myModelMultiple = MyModel_multiple(K, 1)
            myModelMultiple.compile(loss=custom_loss, optimizer=tf.keras.optimizers.legacy.Adam())
            myModelMultiple.fit(inputs_3d_train, output_3d_train, epochs=args.epochs, verbose=0)
        ...
        for m in range(M):
            ...
            inputs_m = np.stack([X_goodbads_test, X_utility_test, W_matrix_m, exposure_matrix_test], axis = -1)
    ...
    path = compose_filename(f"results1006/{args.exp_name}_synthetic_ab_j{J}q{Q}k{K}_100_{uplift_factor}", "csv")
